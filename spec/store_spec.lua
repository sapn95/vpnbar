...
describe("store, settings for the menu as a whole", function()
  it("fills in every default when the file has none", function()
    assert.same({ exclusive = false, fallback = true }, store.settings(store.empty()))
    assert.same({ exclusive = false, fallback = true }, store.settings({}))
    assert.same({ exclusive = false, fallback = true }, store.settings(nil))
  end)

  it("keeps what the file does say and defaults the rest", function()
    local cfg = assert(store.normalise({ settings = { exclusive = true }, profiles = {} }))
    assert.is_true(store.settings(cfg).exclusive)
    assert.is_true(store.settings(cfg).fallback)
  end)

  it("refuses a setting it does not have", function()
    local cfg, err = store.normalise({ settings = { turbo = true }, profiles = {} })
    assert.is_nil(cfg)
    assert.matches("no such setting", err)
  end)

  it("refuses a setting of the wrong type", function()
    local cfg, err = store.normalise({ settings = { exclusive = "yes" }, profiles = {} })
    assert.is_nil(cfg)
    assert.matches("must be boolean", err)
  end)

  it("refuses settings that are not an object at all", function()
    assert.is_nil((store.normalise({ settings = "no", profiles = {} })))
  end)
end)

describe("store.setSettings", function()
  it("returns a new config and leaves the old one alone", function()
    local before = store.empty()
    local after = assert(store.setSettings(before, { exclusive = true }))
    assert.is_false(store.settings(before).exclusive)
    assert.is_true(store.settings(after).exclusive)
  end)

  it("refuses an unknown setting and a wrong type", function()
    assert.is_nil((store.setSettings(store.empty(), { turbo = true })))
