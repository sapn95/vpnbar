...
describe("menu.build", function()
  it("offers disconnect for what is up and connect for what is not", function()
    ...
    -- The switch row and its separator come first with two connections.
    assert.same({ kind = "disconnect", id = "a" }, items[3].action)
    assert.same({ kind = "connect", id = "b" }, items[4].action)
    assert.matches("^●", items[3].title)
  end)
...
describe("menu.build, the switch row", function()
  it("offers the other connection at the top, and does not touch the order", function()
    local cfg = config("a", "b")
    local items = menu.build(cfg, { a = "connected", b = "disconnected" })
    assert.equals("Switch to B", items[1].title)
    assert.same({ kind = "switch", id = "b" }, items[1].action)
    assert.is_true(items[2].separator)
    assert.equals("a", store.list(cfg)[1].id)
  end)

  it("offers the way back once something was switched to", function()
    local items = menu.build(config("a", "b"), {}, "b")
    assert.equals("Switch back to A", items[1].title)
    assert.same({ kind = "switch", id = "a" }, items[1].action)
  end)

  it("lists them in a submenu when there is more than one to switch to", function()
    local items = menu.build(config("a", "b", "c"), {})
    assert.equals("Switch to", items[1].title)
    assert.equals(2, #items[1].menu)
    assert.equals("Switch to B", items[1].menu[1].title)
    assert.equals("Switch to C", items[1].menu[2].title)
  end)

  it("has nothing to offer with one connection", function()
    local items = menu.build(config("a"), {})
    assert.is_nil(find(items, "Switch to"))
    assert.is_true(items[1].title ~= "Switch to A")
  end)

  it("leaves hidden connections out", function()
    local cfg = assert(store.update(config("a", "b"), "b", { hidden = true }))
    assert.is_nil(find(menu.build(cfg, {}), "Switch to B"))
  end)

  it("says so when the current one will stay up", function()
    local plain = menu.build(config("a", "b"), {})
    assert.is_truthy(plain[1].tooltip:find("stays up too", 1, true))
    local exclusive = menu.build(assert(store.setSettings(config("a", "b"), { exclusive = true })), {})
